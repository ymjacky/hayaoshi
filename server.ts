...
export class Server extends EventTarget {
  ...
  constructor() {
    ...
    this.handler = (request: Request, handlerInfo: Deno.ServeHandlerInfo): Response => {
      ...
      const netAddr = handlerInfo.remoteAddr as Deno.NetAddr;
      const remoteHost = netAddr.hostname;
      const userAgent = request.headers.get('user-agent');
      info(`connecting from ${remoteHost}:${netAddr.port} , user-agent: ${userAgent}`);

      const url = new URL(request.url);
      debug('request.url:', url);
      let requestPath = url.pathname;
      let ext = extname(requestPath);
      // const dir = dirname(requestPath);
      // const base = basename(requestPath);
      ...
      if (method === 'GET') {
        ...
        const localFileUrl = new URL(`public${requestPath}`, import.meta.url);
  ...
  listen(port: number) {
    ...
    Deno.serve({ transport: 'tcp', port: port, handler: this.handler });
  }
